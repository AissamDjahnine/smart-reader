...
export default function LibraryHighlightsCenterPanel({
  ...
  return (
    <section
      data-testid="highlights-center-panel"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className={`text-lg font-bold ${isDarkLibraryTheme ? "text-slate-100" : "text-[#1A1A2E]"}`}>Highlights Workspace</h2>
        ...
        <span className={`text-xs font-semibold ${isDarkLibraryTheme ? "text-slate-500" : "text-gray-500"}`}>Manage highlights across books</span>
      </div>

      {highlightsCenterFilteredEntries.length === 0 ? (
        <div
          data-testid="highlights-center-empty"
          ...
          {(highlightsCenterPairs || []).map((pair) => (
            ...
            >
              <div className={`${isDarkLibraryTheme ? "workspace-card-dark" : "workspace-card"} p-3 ${contentPanelHeightClass}`}>
                <div className={`mb-2 text-[11px] uppercase tracking-[0.16em] font-bold ${isDarkLibraryTheme ? "text-indigo-300" : "text-indigo-700"}`}>
                ...
                <div className={`space-y-3 overflow-y-auto pr-1 pb-2 ${contentScrollHeightClass}`}>
                  {pair.entries.map((entry) => (
                    <article
                      ...
                      className={`rounded-xl border p-3 ${isDarkLibraryTheme ? "border-slate-700 bg-slate-900/80" : "border-indigo-100 bg-white"}`}
                    ...
                    >
                      ...
                      <p className={`mt-2 rounded-lg border px-2 py-2 text-sm ${
                        isDarkLibraryTheme
                      ...
                      {entry.note && (
                        <p className={`mt-2 text-xs italic ${isDarkLibraryTheme ? "text-slate-400" : "text-gray-500"}`}>
                          Note: {entry.note}
                        </p>
                      )}
